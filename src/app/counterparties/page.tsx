      ...
      <section>
        ...
        <div className="grid gap-3 md:grid-cols-2">
          {counterparties.map((counterparty) => (
            <article key={counterparty.id} className="rounded-lg border border-line bg-surface p-4">
              ...
              <dl className="mt-4 grid grid-cols-2 gap-3 text-xs">
                <div><dt className="text-ink-3">Configured limit</dt><dd className="mt-0.5 text-ink">{counterparty.baseline_payment_limit == null ? "Not set" : <Money value={counterparty.baseline_payment_limit} />}</dd></div>
                <div><dt className="text-ink-3">Current authority</dt><dd className="mt-0.5 text-ink">{counterparty.payment_limit == null ? "Not set" : <Money value={counterparty.payment_limit} />}</dd></div>
                <div><dt className="text-ink-3">Jurisdiction</dt><dd className="mt-0.5 text-ink">{counterparty.jurisdiction || "Not set"}</dd></div>
                <div><dt className="text-ink-3">Last screened</dt><dd className="mt-0.5 text-ink">{counterparty.last_screened_at ? new Date(counterparty.last_screened_at).toLocaleString() : "Not yet"}</dd></div>
              </dl>
