    ...
    <ul aria-label="Live and simulated product capabilities" className="flex flex-wrap gap-2">
      {legs.map((leg) => (
        <li
          ...
          className={
            leg.live
              ? "flex items-center gap-2 rounded-md border border-proof-line bg-proof-soft px-2.5 py-1.5"
              : "hatch flex items-center gap-2 rounded-md border border-dashed border-line-strong px-2.5 py-1.5"
          }
        >
          <span aria-hidden className={`size-2 shrink-0 rounded-full ${leg.live ? "bg-proof" : "border border-dashed border-ink-3"}`} />
